						...
						if(S_ISDIR(fstat.st_mode)){
								...
								if(isdigit(dirname[0])){
										...
										*p.p_pid=atoi(dentry->d_name);
										...
										p.p_prio=atoi(p_prio);
										p_nice=strtok(NULL," ");
										p.p_nice=atoi(p_nice);
										//printf("%s's prio:%s,nice: %s\n",dirname,p_prio,p_nice);
										...
										//RssFile값 추출, RssSHEmems 추출해서 더해서 SHR에 저장

										char *RssFile;
										char *RssShemem;
										memset(buf2,0,sizeof(buf2));
										strcpy(buf2,buf);
										if(RssFile=strstr(buf2,"RssFile")){
												RssFile=strtok(RssFile,"\t");
												RssFile=strtok(NULL," ");
										}else{
												RssFile="0";
										}

										p.p_SHR=atoi(RssFile);

										memset(buf2,0,sizeof(buf2));
										strcpy(buf2,buf);
										if(RssShemem=strstr(buf2,"RssShmem")){
												RssShemem=strtok(RssShemem,"\t");
												RssShemem=strtok(NULL," ");
										}else{
												RssShemem="0";
										}

										p.p_SHR+=atoi(RssShemem);
...
*/
		...
		for(int i=0;i<ProcInfoIdx;i++){
				printf("%d	%s	%d	%d	%s	%s	%d	%s	%1.1f	%1.1f	%d:%2.2f	%s\n",*ProcInfoArr[i].p_pid,ProcInfoArr[i].p_user,ProcInfoArr[i].p_prio,ProcInfoArr[i].p_nice,ProcInfoArr[i].p_VSZ,ProcInfoArr[i].p_RSS,ProcInfoArr[i].p_SHR,ProcInfoArr[i].p_state,ProcInfoArr[i].p_cpu,ProcInfoArr[i].p_mem,ProcInfoArr[i].p_time_min,ProcInfoArr[i].p_time_sec,ProcInfoArr[i].p_command);
